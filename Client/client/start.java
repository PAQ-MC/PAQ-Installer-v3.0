...
public class start {
	...
	public static void cstart(String mod, String version, boolean preview)
		...
		if(PAQ164.exists()){
			...
			FileUtils.DelateDirectory(new File(AppPath.toString() + "/PAQ/Installer"));
			FileUtils.DelateDirectory(new File(AppPath.toString() + "/PAQ/Launcher"));
			FileUtils.DelateDirectory(new File(AppPath.toString() + "/PAQ/Instances"));
		}
		...
		InstallInfo obj = GetInstallInfo.JsonInfo("http://magetech.no-ip.org/test.json");
